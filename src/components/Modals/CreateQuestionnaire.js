...
const CreateQuestionnaire = ({ isCreateQuestOpen, toggleCreateQuest, setExperimentSurveys, fetch = false }) => {
        ...
        >
            <DialogContent sx={{ backgroundColor: '#f9f9f9', p: { xs: 2, sm: 3 } }}>
                ...
                <form onSubmit={handleSubmit}>
                    <Box sx={{ backgroundColor: '#fff', borderRadius: 2, p: 2, mb: 3, borderTop: '7px solid #0d5086', boxShadow: 1 }}>
                    ...
                    <Box sx={{ display: { xs: 'none', sm: 'flex' }, justifyContent: 'space-between', mt: 2 }}>
                        <Button variant="outlined" onClick={toggleCreateQuest}>{t('cancel')}</Button>
                        <Button type="submit" variant="contained" color="primary" disabled={!isValid || hasEmptyStatement || isLoading}>
                            {isLoading ? <CircularProgress size={24} /> : t('create_survey')}
                        </Button>
                    </Box>
                    <Box sx={{ display: { xs: 'flex', sm: 'none' }, justifyContent: 'space-between', mt: 2 }}>
                        <Button variant="contained" color="primary" onClick={toggleCreateQuest}><CancelOutlined /></Button>
