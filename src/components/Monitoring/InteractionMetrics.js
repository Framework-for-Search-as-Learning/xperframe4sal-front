              ...
              {tasksExecution.map((task) => {
                ...
                if (taskType === 'search-engine') {
                  ...
                  const totalQueries = task.executions.reduce(
                    (sum, ex) => sum + (ex.searchDetails?.queriesCount || 0),
                    0,
                  );
                  ...
                  const totalMessages = task.executions.reduce(
                    (sum, ex) => sum + (ex.llmDetails?.totalMessages || 0),
                    0,
                  );
...
const ByTaskTab = ({
  ...
  const avgTime =
    taskData.executions.length > 0
      ? taskData.executions.reduce((sum, ex) => sum + (ex.executionTime || 0), 0) /
  ...
  const totalQueries = isSearchTask
    ? taskData.executions.reduce((sum, ex) => sum + (ex.searchDetails?.queriesCount || 0), 0)
    : 0;

  const totalMessages = !isSearchTask
    ? taskData.executions.reduce((sum, ex) => sum + (ex.llmDetails?.totalMessages || 0), 0)
    : 0;

  const executionTimeData = taskData.executions.map((exec, idx) => ({
    name: `Ex ${idx + 1}`,
    tempo: Math.round((exec.executionTime || 0) / 1000),
  }));
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                {isSearchTask && (
                  <>
                    <TableCell align="center">{t('resources') || 'Recursos'}</TableCell>
                    <TableCell align="center">{t('queries') || 'Consultas'}</TableCell>
                  </>
                )}
                {!isSearchTask && (
                  <TableCell align="center">{t('messages') || 'Mensagens'}</TableCell>
                )}
              </TableRow>
            </TableHead>
            <TableBody>
              {taskData.executions.map((exec, idx) => (
                <TableRow key={exec.userTaskId || idx}>
                  ...
                  {!isSearchTask && (
                    <TableCell align="center">{exec.llmDetails?.totalMessages || 0}</TableCell>
                  )}
                </TableRow>
              ))}
            </TableBody>
              ...
              <Accordion key={task.taskId} defaultExpanded sx={{ mb: 3, boxShadow: 2 }}>
                <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    ...
                    <Chip
                      label={
                        taskDetails[0]?.taskType === 'search-engine'
                          ? t('search') || 'Busca'
                          : t('chat') || 'Chat'
                      }
                  ...
                  {taskDetails.map((detail) => {
                    ...
                    return (
                      <Box key={detail.userTaskId} sx={{ mb: 5 }}>
                            ...
                            <>
                              <Grid item xs={6} sm={4}>
                                <Paper variant="outlined" sx={{ p: 2, textAlign: 'center' }}>
                                  ...
                                  <Typography variant="h6">
                                    {detail.searchDetails?.resourcesAccessedTotal || 0}
                                  </Typography>
                                </Paper>
                              </Grid>
                              <Grid item xs={6} sm={4}>
                                <Paper variant="outlined" sx={{ p: 2, textAlign: 'center' }}>
                                  ...
                                  <Typography variant="h6">
                                    {detail.searchDetails?.queriesCount || 0}
                                  </Typography>
                                </Paper>
                              </Grid>
                            </>
                            ...
                            <>
                              <Grid item xs={6} sm={4}>
                                <Paper variant="outlined" sx={{ p: 2, textAlign: 'center' }}>
                                  ...
                                  <Typography variant="h6">
                                    {detail.llmDetails?.totalMessages || 0}
                                  </Typography>
                                </Paper>
                              </Grid>
                              <Grid item xs={6} sm={4}>
                                <Paper variant="outlined" sx={{ p: 2, textAlign: 'center' }}>
                                  ...
                                  <Typography variant="h6">
                                    {detail.llmDetails?.promptsCount || 0}
                                  </Typography>
                                </Paper>
                              </Grid>
                            </>
                                ...
                                <Table size="small">
                                  <TableHead sx={{ bgcolor: '#f0f4f8' }}>
                                    <TableRow>
                                      ...
                                      <TableCell sx={{ fontWeight: 'bold' }}>
                                        {t('exact_time') || 'Hora (HH:MM:SS)'}
                                      </TableCell>
                                      <TableCell sx={{ fontWeight: 'bold' }}>
                                        {t('resource_title') || 'Título do Recurso'}
                                      </TableCell>
                                      <TableCell sx={{ fontWeight: 'bold' }}>
                                        {t('source_query') || 'Consulta de Origem'}
                                      </TableCell>
                                      <TableCell align="right" sx={{ fontWeight: 'bold' }}>
                                        {t('time_spent') || 'Permanência'}
                                      </TableCell>
                                    </TableRow>
                                  </TableHead>
                                  <TableBody>
                                ...
                                >
                                  <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                                    ...
                                    >
                                      ...
                                      >
                                        {q.resourcesAccessedCount || 0}{' '}
                                        {t('links_accessed') || 'links acessados'}
                                      </Typography>
                                    </Box>
                                  </AccordionSummary>
                                      ...
                                      <Table size="small">
                                        <TableHead sx={{ bgcolor: '#fafafa' }}>
                                          <TableRow>
                                            ...
                                            >
                                              {t('time_spent_header') || 'PERMANÊNCIA'}
                                            </TableCell>
                                            <TableCell
                                              align="right"
                                              sx={{
                                        ...
                                        <TableBody>
                                          {q.resources?.map((resource, rIdx) => (
                                            <TableRow key={rIdx} hover>
                                              ...
                                              <TableCell align="right" sx={{ fontSize: '0.75rem' }}>
                                                {new Date(resource.visitTime).toLocaleTimeString(
                                                  'en-US',
                                                  {
                                                    hour: '2-digit',
                                                    minute: '2-digit',
                        ...
                        {detail.taskType === 'llm' &&
                          detail.llmDetails?.messages &&
                          detail.llmDetails.messages.length > 0 && (
                            <Box sx={{ mb: 4 }}>
                              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, gap: 1 }}>
