  ...
  const handleExportParticipants = async () => {
    ...
    try {
      const csvContent = [
        ...
        ...participants.map((p) => {
          const startDate = p.startDate ? new Date(p.startDate).toLocaleString('pt-BR') : '';
            ...
            {participants.map((participant) => (
              <TableRow key={participant.id}>
                ...
                <TableCell align="center">
                  {participant.progress !== undefined ? `${participant.progress.toFixed(1)}%` : '-'}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
        ...
        <Box sx={{ mt: 3, p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
          ...
          <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
            ...
            <Typography variant="body2" color="success.main">
              <strong>{t('finished') || 'Finalizados'}:</strong> {stats.finishedParticipants}
            </Typography>
            <Typography variant="body2">
