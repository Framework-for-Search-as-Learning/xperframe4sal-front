...
const EditGroupArea = ({ ExperimentId }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setUsersInExperiment(
        usersInExperimentData.filter(
          (userInExperiment) =>
  ...
  const removeUserFromGroup = async (userId) => {
    try {
      const group = groups.find((group) => group.users.some((user) => user._id === userId));

      await api.delete(`/user-task?userId=${userId}&taskId=${group.id}`, {
        headers: { Authorization: `Bearer ${user.accessToken}` },
      });
