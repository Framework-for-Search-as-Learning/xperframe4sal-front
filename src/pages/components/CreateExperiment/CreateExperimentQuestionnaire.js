    ...
    const handleSaveEdit = async (updatedSurvey) => {
        if (isEditMode) {
            try {
                ...
                setFeedback({ open: true, message: t('success_edit') || "Questionário salvo com sucesso!", severity: 'success' });
            } catch (error) {
                console.error("Erro ao atualizar o questionário:", error);
                setFeedback({ open: true, message: t('error') || "Erro ao salvar o questionário.", severity: 'error' });
                return;
            }
        }
    ...
    const handleConfirmDelete = async () => {
        if (isEditMode) {
            try {
                ...
                setFeedback({ open: true, message: t('success_delete') || "Questionário excluído!", severity: 'success' });
            } catch (error) {
    ...
    return (
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', marginTop: 5 }}>
            <Box sx={{ width: { xs: '100%', sm: '60%' }, padding: { xs: 1, sm: 3 }, display: 'flex', flexDirection: 'column', backgroundColor: '#f9f9f9', borderRadius: '8px', boxShadow: 4, mx: 'auto' }}>
                ...
                {Array.isArray(ExperimentSurveys) && ExperimentSurveys.length > 0 ? (
                    <FormControl fullWidth sx={{ minHeight: 300, maxHeight: 300, overflowY: 'auto' }}>
                        {ExperimentSurveys.map((survey, index) => (
                            <Box key={index} sx={{ display: 'flex', flexDirection: 'column', mb: 1, p: 1, backgroundColor: '#fff', borderRadius: '4px', boxShadow: 1, wordBreak: 'break-word', '&:hover': { backgroundColor: '#e6f7ff' } }}>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <ListItemText primary={survey.title} sx={{ ml: 1 }} />
                                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                        <IconButton color="error" onClick={() => setDeleteTarget(index)}><DeleteIcon /></IconButton>
                                        <IconButton color="primary" onClick={() => setEditTarget({ index, survey })}><EditIcon /></IconButton>
                                ...
                                {openDescIds.includes(index) && (
                                    <Box sx={{ p: 1, backgroundColor: '#E8E8E8', borderRadius: '4px', maxHeight: 150, overflowY: 'auto' }} dangerouslySetInnerHTML={{ __html: survey.description }} />
                                )}
                            </Box>
                        ))}
                    </FormControl>
                ...
                <Box sx={{ display: { xs: 'none', sm: 'flex' }, justifyContent: isEditMode ? 'flex-end' : 'space-between', mt: 2 }}>
                ...
                <Box sx={{ display: { xs: 'flex', sm: 'none' }, justifyContent: isEditMode ? 'center' : 'space-between', mt: 2 }}>
                    {!isEditMode && <Button variant="contained" onClick={() => setStep(step - 1)}><ArrowBack /></Button>}
                    <Button variant="contained" onClick={() => setIsCreateOpen(true)}>{t('create_survey')}</Button>
                    {!isEditMode && <Button variant="contained" onClick={() => setStep(step + 1)}><ArrowForward /></Button>}
                </Box>
            </Box>

            <Dialog open={deleteTarget !== null} onClose={() => setDeleteTarget(null)} maxWidth="xs" fullWidth sx={{ '& .MuiDialog-paper': { borderRadius: '12px', p: 4 } }}>
