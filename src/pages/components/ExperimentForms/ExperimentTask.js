...
const ExperimentTask = () => {
  ...
  const handleDeleteTask = async () => {
    ...
    if (isEditMode) {
      try {
        ...
        setFeedback({
          open: true,
          message: t('Success_Delete') || 'Tarefa excluída!',
          severity: 'success',
        });
      } catch (error) {
  ...
  const handleCreateTask = async (e) => {
    ...
    if (isEditMode) {
      try {
        ...
        setFeedback({
          open: true,
          message: t('success_create') || 'Tarefa criada!',
          severity: 'success',
        });
      } catch (error) {
  ...
  const handleEditTaskSubmit = async (e) => {
    ...
    if (isEditMode) {
      try {
        ...
        setFeedback({
          open: true,
          message: t('success_edit') || 'Tarefa salva!',
          severity: 'success',
        });
      } catch (error) {
  ...
  const handleQuestionChangeGeneric = (event, isEdit) => {
    ...
    if (isEdit) {
      ...
      const selectedQuestions = createForm.formState.SelectedSurvey.questions?.filter((q) =>
