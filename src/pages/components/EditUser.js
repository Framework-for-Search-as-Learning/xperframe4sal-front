...
const EditUser = () => {
  ...
  const displayStep = () => {
    if (actualStep === 0) return <EditUserArea ExperimentId={{ experimentId }} />;
    if (actualStep === 1) return <EditGroupArea ExperimentId={{ experimentId }} />;
          ...
          <div className={styles.stepContainer}>
            {STEPS.map((step, index) => (
              ...
              >
                ...
                <p className={index !== actualStep ? styles.inactiveText : undefined}>
                  {t(step.label)}
                </p>
              </div>
            ))}
