...
const ICF = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        let experiment = (
          await api.get(`experiment/${experimentId}`, {
            headers: {
        ...
        if (!userExperiment) {
          ...
          if (userExperimentsData?.stepsCompleted?.['icf']) {
    ...
    <Paper elevation={3} style={{ padding: '16px' }}>
      <Container elevation={3} maxWidth="md">
        <Paper elevation={3} style={{ padding: '16px' }}>
          ...
          />
          <Button variant="contained" color="primary" onClick={handleChange} disabled={!accepted}>
            {t('accept_button')}
          </Button>
        </Paper>
      </Container>
