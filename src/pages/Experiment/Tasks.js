  ...
  const fetchTaskData = useCallback(async () => {
    try {
      ...
      const [userExpRes, userTasksRes, stepsRes] = await Promise.all([
        ...
        api.get(`experiment/${experimentId}/step`, {
          headers: { Authorization: `Bearer ${user.accessToken}` },
        }),
      ]);
  ...
  const handleTaskAction = async (taskId, actionType) => {
    try {
      const userTask = userTasks.find(
        (ut) => ut.task?._id === taskId || ut.task_id === taskId || ut.taskId === taskId,
      );
  ...
  return (
    <ExperimentTemplate headerTitle={t('see_tasks_list_title')} steps={steps}>
      ...
      {tasks?.map((task, index) => {
        const userTask = userTasks.find(
          (ut) => ut.task?._id === task._id || ut.task_id === task._id,
        );
          ...
          <Accordion key={task._id} elevation={3} defaultExpanded={true} disabled={hasFinished}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography>
                ...
                {hasFinished && (
                  <span style={{ color: 'red', marginLeft: '10px' }}>- {t('task_finished')}</span>
                )}
              </Typography>
            </AccordionSummary>
            <Divider />
