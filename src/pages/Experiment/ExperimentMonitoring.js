...
const ExperimentMonitoring = () => {
  ...
  const {
    ...
    isAuthorized,
    data: authData,
  } = useExperimentAuth(experimentId, user);
  ...
  const loadMonitoringData = useCallback(async () => {
    ...
    try {
      ...
      const [stats, participants, surveysStats, tasksExecution] = await Promise.all([
        api.get(`experiment/${experimentId}/stats`, {
          headers: { Authorization: `Bearer ${user.accessToken}` },
        }),
        api.get(`experiment/${experimentId}/participants`, {
          headers: { Authorization: `Bearer ${user.accessToken}` },
        }),
        api.get(`experiment/${experimentId}/surveys-stats`, {
          headers: { Authorization: `Bearer ${user.accessToken}` },
        }),
        api.get(`experiment/${experimentId}/tasks-execution`, {
          headers: { Authorization: `Bearer ${user.accessToken}` },
        }),
      ]);
      ...
      setError(t('error_loading_monitoring') || 'Erro ao carregar dados de monitoramento');
    } finally {
      setLoading(false);
    }
