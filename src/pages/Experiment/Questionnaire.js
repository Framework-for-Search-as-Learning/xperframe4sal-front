  ...
  try {
    if (userSurveysApiCalls.length > 0) {
      ...
      if (answeredSurveys.length === userSurveysApiCalls.length) {
        userExperiment.stepsCompleted = Object.assign(userExperiment.stepsCompleted, {
          [stepName]: true,
        });
...
async function separateUsersInGroup(api, user, userScore, experiment) {
  if (userScore) {
    ...
    const response = await api.get(`user-task?userId=${user.id}&taskId=${taskId}`, {
      headers: { Authorization: `Bearer ${user.accessToken}` },
    });
...
const Questionnaire = () => {
  ...
  useEffect(() => {
    ...
    const fetchSurveyData = async () => {
      setIsLoading(true);
      try {
        const [userExperimentResponse, experimentResponse, surveyResponse, userSurveyResponse] =
          await Promise.all([
            ...
            !survey
              ...
              : null,
            api.get(`survey-answer?userId=${user.id}&surveyId=${surveyId}`, {
              headers: {
                Authorization: `Bearer ${user.accessToken}`,
              },
            }),
    ...
    try {
      ...
      if (surveyAnswer) {
        ...
        await handleSurveySubmit(() =>
          api.patch(`survey-answer/${surveyAnswer._id}`, surveyAnswer, {
            headers: {
              Authorization: `Bearer ${user.accessToken}`,
            },
          }),
